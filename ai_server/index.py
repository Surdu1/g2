...
def verification():
    ...
    for clip in all_video:
        ...
        for comentariu in comentari_object:
            comentari_count += 1
        time = clip["time"]
        categorizat.append({"id": clip["id"],"description": clip["description"],"time": int(time),"like": like_count,"view": view_count,"comentari": comentari_count})
    ...
    sorted_arr_id = []
    for sort in sorted_arr:
        sorted_arr_id.append({"id":sort["id"],"description":sort["description"]})
    return sorted_arr_id
...
@app.route('/get_video_complet',methods=['POST'])
def get_video_complet():
    sorted_arr = verification()
    return jsonify(sorted_arr)
