...
app.get('/api/video/front/:video',async(req,res)=>{
    if(req.params.video){
        ...
        .then((screenshotData) => {
            const base64 = fromByteArray(new Uint8Array(screenshotData));
        ...
        console.error('Error capturing screenshot:', error);
         ...
         res.json({thumbnail:file.thumbnail})
        }
    }
})
...
app.post('/search', async(req,res)=>{
    ...
    for(let i = 0; i < resu.length; i++) {
        result.push({id:resu[i].id,description:resu[i].description});
    }
