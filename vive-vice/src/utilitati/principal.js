...
export default function Principal(){
    ...
    async function init(){
       const result =  await axios.post("http://localhost:8000/get_video_complet");
       setData(result.data)
    ...
    {data? data.map((element,i) => {
        ...
        return(<PrincipalVideo key={i} element={element.id} description={element.description}></PrincipalVideo>)
    }): ""}</div> : <div className="cards">{(Cookies.get("token"))?  <NewOne/> : ""}
    {data1?data1.map((element,i) => {
        return(<PrincipalVideo key={i} element={element.id} description={element.description}></PrincipalVideo>)
    }):""}</div> }
